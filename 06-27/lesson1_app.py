"""
正弦與餘弦波形繪圖互動應用程式
使用 numpy 與 matplotlib，搭配滑桿即時調整振幅、頻率與相位偏移
"""

import numpy as np
import matplotlib.pyplot as plt
from matplotlib.widgets import Slider

# ============================================================
# 設定中文字型（macOS 使用 Arial Unicode MS 或 Heiti TC；
# Windows 可使用 Microsoft JhengHei）
# ============================================================
plt.rcParams['font.sans-serif'] = ['Arial Unicode MS', 'Heiti TC', 'Microsoft JhengHei']
plt.rcParams['axes.unicode_minus'] = False  # 避免負號顯示異常

# ============================================================
# 建立圖表與軸
# ============================================================
fig, ax = plt.subplots(figsize=(10, 6))
plt.subplots_adjust(bottom=0.3)  # 預留下方空間給滑桿

# X 軸範圍：0 ~ 4π
x = np.linspace(0, 4 * np.pi, 1000)

# ============================================================
# 初始參數
# ============================================================
A_init = 1.0      # 振幅
omega_init = 1.0  # 頻率
phi_init = 0.0    # 相位偏移

# ============================================================
# 繪製初始波形
# ============================================================
sin_line, = ax.plot(x, A_init * np.sin(omega_init * x + phi_init),
                    label='sin(ωx + φ)', linewidth=2)
cos_line, = ax.plot(x, A_init * np.cos(omega_init * x + phi_init),
                    label='cos(ωx + φ)', linewidth=2, linestyle='--')

# ============================================================
# 設定圖表外觀（繁體中文）
# ============================================================
ax.set_title('正弦（sin）與餘弦（cos）波形圖', fontsize=14)
ax.set_xlabel('x', fontsize=12)
ax.set_ylabel('y', fontsize=12)
ax.set_xlim(0, 4 * np.pi)
ax.set_ylim(-5.5, 5.5)
ax.grid(True, linestyle=':', alpha=0.6)
ax.legend(loc='upper right')

# 在 X 軸標示 π 刻度
ax.set_xticks([0, np.pi, 2 * np.pi, 3 * np.pi, 4 * np.pi])
ax.set_xticklabels(['0', 'π', '2π', '3π', '4π'])

# ============================================================
# 建立滑桿（Slider）
# ============================================================
# 滑桿顏色
slider_color = 'lightgoldenrodyellow'

# 振幅滑桿
ax_amp = plt.axes([0.15, 0.20, 0.70, 0.03], facecolor=slider_color)
slider_amp = Slider(
    ax=ax_amp, label='振幅 (A)', valmin=0.1, valmax=5.0,
    valinit=A_init, valstep=0.05
)

# 頻率滑桿
ax_freq = plt.axes([0.15, 0.13, 0.70, 0.03], facecolor=slider_color)
slider_freq = Slider(
    ax=ax_freq, label='頻率 (ω)', valmin=0.1, valmax=10.0,
    valinit=omega_init, valstep=0.05
)

# 相位偏移滑桿
ax_phase = plt.axes([0.15, 0.06, 0.70, 0.03], facecolor=slider_color)
slider_phase = Slider(
    ax=ax_phase, label='相位偏移 (φ)', valmin=0, valmax=2 * np.pi,
    valinit=phi_init, valstep=0.01
)


# ============================================================
# 更新回呼函數：當滑桿數值變動時重新繪製波形
# ============================================================
def update(val):
    A = slider_amp.val
    omega = slider_freq.val
    phi = slider_phase.val

    sin_line.set_ydata(A * np.sin(omega * x + phi))
    cos_line.set_ydata(A * np.cos(omega * x + phi))

    fig.canvas.draw_idle()  # 非阻塞式重新繪圖


# 將三個滑桿的變動事件綁定到 update 函數
slider_amp.on_changed(update)
slider_freq.on_changed(update)
slider_phase.on_changed(update)

# ============================================================
# 顯示圖表（互動視窗）
# ============================================================
plt.show()
